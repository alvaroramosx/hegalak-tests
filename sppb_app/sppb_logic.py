...
def score_balance(feet_together_s: float, semi_tandem_s: float, tandem_s: float) -> int:
	...
	Rules used (widely adopted SPPB cutoffs):
	- If cannot hold feet together for 10s -> 0
	- If feet together 10s but semi-tandem <10s -> 1
	- If semi-tandem 10s but tandem <3s -> 2
	- If tandem held 3-9.99s -> 3
	- If tandem held for full 10s -> 4
	"""
...
def interpret_total(total_score: int) -> str:
	"""Return brief interpretation: fragilidad baja/moderada/alta."""
